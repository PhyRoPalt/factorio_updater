...
if [ $experimental = true ] 
		then 
			got_version=$(wget --no-cache -qO - https://www.factorio.com/download-headless/experimental | awk 'BEGIN { findstr="/headless/linux64";}{if (match($0, findstr)) {theend=RSTART ; {if (match($0,"/get-download/")) {thestart=RSTART; thestartl=RLENGTH; theversion=substr($0,thestart+thestartl, theend-thestart-thestartl)}}}; if (match($theversion,"latest") {nextsearch=substr($0,theend);if (match($nextsearch, findstr)) {theend=RSTART ; {if (match($nextsearch,"/get-download/")) {thestart=RSTART; thestartl=RLENGTH; theversion=substr($nextsearch,thestart+thestartl, theend-thestart-thestartl)}}}};printf("%s", theversion);exit;}')
		else
